...
from PyQt4.QtGui import QPlainTextEdit #import gtk, gobject
...
class ConsoleView(QPlainTextEdit):
  ...
  def __init__(self, parent=None):
    ...
    #self.connect(self, SIGNAL("key-press-event"), self.onKeyPress)
#self.connect('key-press-event', self.onKeyPress)
  ...
  def _showPrompt(self, prompt):
    ...
    @type prompt: string
    '''
    print >>STDOUT,'Writing prompt',prompt
    self._write(prompt)
  ...
  def getCurrentLine(self):
    ...
    @rtype: string
    '''
    rv = self.textCursor().block().text()
    # Hack off any prompt
    rv = rv.section(':',1)
    print >>STDOUT,'Text',str(rv)
    return str(rv)
...
#    self.connect('key_press_event', self.keyPress)
  ...
  def keyPressEvent(self, event):
    ...
    if event.key() == Qt.Key_Return:
      self._processLine()
    ConsoleView.keyPressEvent(self, event)
